...
class LinkjoyAncestorGenericDuelFinancialUneven
  ...
  static showViewer(BuildContext context, LinkjoyOfour snap) {
    LinkjoyGenericDuelDefinitive.show(
      context,
      LinkjoyAncestorGenericDuelFinancialUneven(snap),
    );
  }
  ...
  LinkjoyAncestorGenericDuelFinancialUneven(this.snap) : super() {
    super.isHero = true;
    super.itemCount = 1;
  ...
  ImageProvider imageProvider(int index) {
    ...
    if (_isLocalMediaNotOutOfDate(item.createTime!) &&
        !LinkjoyGnomeSister.isEmptyString(item.image?.relativePath)) {
      return FileImage(File(item.image!.absolutePath!));
    } else {
      if (item.image!.width! < LINKJOY.linkjoyShock.imageSize.large &&
          item.image!.height! < LINKJOY.linkjoyShock.imageSize.large) {
        return LinkjoyFastenDance.provider(item.image!.imgUrl!);
      } else {
